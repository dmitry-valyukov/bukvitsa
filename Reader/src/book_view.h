...
private:
    ...
    /// освобождается через долгую паузу (вдруг читатель ещё листнёт), дальше по
    /// одному в секунду, пока пул не опустеет. Возобновление листания
    /// (`acquireFlip`) таймер снимает.
    void armRelease();
