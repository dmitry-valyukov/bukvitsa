...
private:
    /// Один идущий переворот: лист с уходящим разворотом и всё, что этому листу
    /// нужно, чтобы перевернуться. Плоское листание двигает `sheet` (тень —
    /// `shadow`); книжное режет его к корешку (`clip`), поднимает приходящий
    /// лист (`leaf` в окне `leafClip`, с полутоном изгиба `bend` — ребёнком
    /// leaf) и кладёт тени сгиба (`fold`) и наружного края (`edge`).
    ///
    ...
    struct Flip {
        std::optional<wxl::DrawingSurface> surface;   ///< уходящий разворот (в optional: нет пустого ctor)
        wxl::SpriteVisual sheet;       ///< показывает surface
        ...
        wxl::SpriteVisual leaf;        ///< приходящий лист (книжное); кисть — задник
    ...
    /// захватывает уходящий разворот на лист, двигает книгу на целевой (задник),
    /// запускает анимацию листа. Целевая колонка может лежать в соседней главе —
    /// тогда та становится текущей, не теряя уже разложенной вёрстки.
    ...
    /// держит РАЗВОРОТ ПОСЛЕДНЕГО ОСЕВШЕГО ЛИСТА, а не самый новый: его старая
    /// неперелистываемая половина обязана оставаться на виду, пока лист летит.
    /// На новый разворот он переезжает при оседании (settleSheets) — тогда флаг
    /// backdropStale_ и снимается.
