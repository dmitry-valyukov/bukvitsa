...
StartScreen::StartScreen(const Compositor& compositor) : compositor_(compositor) {
    ...
    //
    // Заставка живёт дважды, и это осознанное промежуточное состояние: в
    // покое её рисует этот Image, а в просветах быстрого ресайза — задник
    // окна (window_backdrop_image в main.cpp) с тем же кадрированием. Убрать
    // Image нельзя: грунт острова WinUI-окна непрозрачен, и без него виден
    // белый лист. Двойной вывод уйдёт с переездом на своё окно верхнего
    // уровня — см. docs/decisions.md, «Окно Reader: своё, с островами».
    //
