...
StartScreen::StartScreen(const Compositor& compositor) : compositor_(compositor) {
    ...
    // Заставки среди детей нет: её несёт задник окна (window_backdrop_image
    // в main.cpp), нарисованный в поверхность под островом XAML с тем же
    // кадрированием. Рисовать её ещё и элементом Image значило бы выводить
    // тяжёлую картинку дважды на каждый шаг ресайза — а прозрачный остров
    // и так показывает задник. Карточка кнопок — единственное, что остров
    // держит сам.
    root_ = Grid{
