...
wxl::Teardown wxl_launched() {
    ...
    // Заставка — ещё и слоем позади острова XAML, как в песочнице
    // WindowsCompositor: цель системного композитора прямо на HWND, размер
    // выставляется синхронно в WM_SIZE. Остров при быстрой растяжке отстаёт
    // от рамки на такт-другой, и обнажённую полосу прежде заливал белым его
    // каркас — теперь в ней продолжается та же картинка. Выравнивание кисти
    // совпадает с XAML-заставкой (UniformToFill к верхней середине), поэтому
    // два слоя сходятся пиксель в пиксель.
